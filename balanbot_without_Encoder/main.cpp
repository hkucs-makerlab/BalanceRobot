...
Identity id;    // Identification used in UDP connection
UDPClass udp;   
...
PID_t speedPID[2];  //PID Controllers for wheel position
...
float motor_min = 6.0f;
float zero = -5.7f;  // bias of imu when keeping car upright
int counter = 0;    // left Encoder value
int counter2 = 0;   // right Encoder value
float driveTrim = 0;    // receive Drive command from user
float turnTrim = 0;     // receive Turn command from user
float maxDrive = 60;    // maximum allowed drive value
...
void getOrientation(float &pitch,float &roll) {
    ...
    if(VERSION == 1)                 
        ...
        i2cRoll = (float) atan2(az, -ay);  // ay -> za, az -> -ay
...
void* readEncoder(void* prt){
    encoder.initEncoders(EA1,EA2,EB1,EB2);
    double* temp;
    temp = new double[2];
    while(true){     
...
// map value in range (min_1 ~ max_1) to range (min_2 ~max_2)
...
int main(int argc, char** argv) {
    ...
    float LeftStep(0), RightStep(0);    // left/right motor step counter
    ...
    while (running) {
        ...
        if(VERSION == 1){
            ...
            std::cout << "angle:  "<< kalmanAngle - zero << " \r" << endl;
        }
     
        //std::cout << "Kalman_Angle:   " << zero - kalmanAngle << "\r\n";
        ...
        if(!inFalloverState){
...
#ifndef DISABLE_ENCODERS
            speedPIDoutput[0] = controller.PIDUpdate(0, counter, during, &speedPID[0]);
            speedPIDoutput[1] = controller.PIDUpdate(0, counter2, during, &speedPID[1]);
#else
            speedPIDoutput[0] = controller.PIDUpdate(0, LeftStep/2, during, &speedPID[0]);
            speedPIDoutput[1] = controller.PIDUpdate(0, RightStep/2, during, &speedPID[1]);
#endif      
...
#endif
            oriOutput = controller.PIDUpdate(0, posDiff, during, &oriPID);
            ...
            //std::cout << "target:  "<< speedPIDoutput[0] << " \r" << endl;
           ...
           */
            ...
            // update steps of motors
            LeftStep -= pitchPIDoutput[0];
            RightStep -= pitchPIDoutput[1];
            //std::cout << "left: " << LeftStep << "   right: " << RightStep << " \r" << endl;
